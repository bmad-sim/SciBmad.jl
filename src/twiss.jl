...
function _twiss_2(n_steps_saved, v0_and_coast, GTPSA_descriptor, ::Val{spin}, ::Val{RDTs}) where {spin, RDTs}
  v0 = v0_and_coast[1]
  ...
  eye = DAMap(init=init, nv=nv, np=np, v0=view(v0, :, 1:nv), v=I, q=(spin ? I : nothing))
  maps = @noinline _twiss_2_preallocate(n_steps_saved, eye)
  return eye, maps, zero_LF, zero_phase, zero_orbit, zero_h
end

function _twiss_2_preallocate(n_steps_saved, map::T) where {T<:DAMap}
  maps = Vector{T}(undef, n_steps_saved)
  for i in 1:n_steps_saved
...
function _twiss_3(_step_save, _maps)
  let step_save=_step_save, maps=_maps, curstep=Ref{Int}(0), cur_step_save_idx=Ref{Int}(1)
    return (coords, ds_step, g) -> begin
      curstep[] += 1
      if curstep[] == step_save[cur_step_save_idx[]] # Store the current map
        map = maps[cur_step_save_idx[]]
        nv = NNF.nvars(map)
        NNF.setray!(map.v, v=reshape(coords.v, 1:nv))
        ...
        NNF.setray!(coords.v, scalar=NNF.getscalar(map), v_matrix=I)
...
function _twiss_4!(eye, cb, zero_t)
  ...
  b0 = Bunch(v=v, q=q, callbacks=(cb,), t_ref=zero_t, p_over_q_ref=b)
...
function _twiss_5(eye, maps)
  # At this point, eye contains the map from the last written place to the end
...
function _twiss_6(m::DAMap, mo, coasting_beam)
  a = normal(m)
...
function twiss(
  ...
  eye, maps, zero_LF, zero_phase, zero_orbit, zero_h = _twiss_2(length(step_save), v0_and_coast, GTPSA_descriptor, Val{spin}(), Val{RDTs}())
  
  # Type stable steps:
  return _twiss_type_stable(eye, maps, s, names, idxs, step_save, zero_LF, zero_phase, zero_orbit, zero_h, Val{de_moivre}(), Val{normalizing_map}())
end
