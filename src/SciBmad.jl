...
function twiss(bl::Beamline; GTPSA_descriptor=GTPSA.desc_current)
  ...
  m = DAMap(v=b0.v)
  # function barrier
  return _twiss(m, b0, bl, Val{linear}())
end
function _twiss(m::DAMap, b0::Bunch, bl::Beamline, ::Val{linear}) where {linear}
  N_ele = length(bl.line)
  a = normal(m)
  r_cs = fast_canonize(a)
  a = a ∘ r_cs
  #a0, a1, a2 = factorize(a)
  lf1 = compute_lattice_functions(a, Val{linear}())
  phase = zeros(3, N_ele+1)
  s = zeros(N_ele+1)
  lf = Vector{typeof(lf1)}(undef, N_ele+1)
  lf[1] = lf1
  b0 = Bunch(view(a.v, 1:6), mem=BeamTracking.AoS, Brho_ref=bl.Brho_ref)
  for i in 1:N_ele
    b0.v .= view(a.v, 1:6)
    track!(b0, bl.line[i])
    phase[:,i+1] .+= phase[:,i]
    s[i+1] = s[i] + bl.line[i].L
    r_cs = fast_canonize(a; phase=view(phase, :, i+1))
    a = a ∘ r_cs
    #a0, a1, a2 = factorize(a)
    lf[i+1] = compute_lattice_functions(a, Val{linear}())
  end
